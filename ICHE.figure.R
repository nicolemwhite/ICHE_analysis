...
# 3) Many Locations, random change time, with interim period
data3 = expand.grid(Location=LETTERS[1:8], Time=1:6)
Period = data.frame(Location=LETTERS[1:8], Change=sample(2:5, size=8, replace=T))
data3 = merge(data3, Period, by='Location')
data3$Period = as.numeric(data3$Time == data3$Change) + 1 # during
data3$Period[data3$Time > data3$Change] = 3 # add one to allow "during change"
data3$Period = factor(data3$Period, levels=1:3, labels=c('Before\nchange','During\nchange','After\nchange')) # for ordering
...
tplot2 = ggplot(data=data2, aes(x=Time, y=Location, fill=Period))+
  ...
  theme(panel.grid.minor = element_blank(), legend.position='none',
        axis.title.x=element_blank(), axis.title.y=element_blank())
tplot2
# panel 3 - with legend so it can be extracted
...
grid.arrange(legend, arrangeGrob(tplot1,tplot2,tplot3, ncol=3), 
